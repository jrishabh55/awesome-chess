...
- Opening Teacher with complete opening courses, sections grouped by shared starting moves, explanations for both sides, middlegame plans, cumulative recall drills, points, and saved progress.
...
- **Opening Teacher:** choose **Openings** in the left menu and use the compact **Opening course** search. Selecting London, Sicilian, or another main opening opens its entire course. The syllabus groups related variations by their shared starting moves, with a foundation first. Duplicate move sequences and intermediate prefixes are combined; complete database courses are not capped at 40 variations. Every guided move explains what White or Black is doing. Use Left / Right, the navigation buttons, or follow the arrow on the board, then read the middlegame plans at the end of the variation. **Why this move** and **Read full plan** open the complete explanation without adding scrolling to the main screen.
- **Course practice:** after variation 1, recall that variation; after variation 2, recall both in shuffled order; after variation 3, recall all three, and so on. Each learned variation appears once per round. Drills hide arrows and future moves until you request **Hint**; incorrect moves are flagged and must be corrected. Completing a drill earns **10 points clean, or 5 after a mistake**. **Retry drill** can upgrade that drill to 10 without stacking points; hints are recorded but do not reduce the score. Continue automatically to the next lesson after the round. The final round covers the whole course. Switching courses preserves each one's progress, scores, and current drill on this device, including offline. A stale tab offers **Load saved progress** instead of overwriting newer results.
- **Learned:** the opening chooser keeps a separate history of courses you have started while leaving them available in **Courses**. Cards and search results show your learned variation count or a Completed badge. Expand a course section to see which variations you have recalled, which need a clean retry, and your current lesson. Each variation counts once even across cumulative drills. Completing the final cumulative round marks the whole course complete; restarting lessons preserves this history and your best scores. The compact chooser keeps its title and tabs visible, with scrolling inside the content area.
- **Custom opening practice:** the chooser's **Single lines**, **My repertoires**, and **Import PGN** tabs retain individual White/Black line practice, saved personal collections (up to 40 lines), and PGN variations. **Short guided lessons** contains the original authored Italian, Queen's Gambit, and Caro-Kann packs. These custom/short packs keep their simpler tour-and-drill scheduler; the complete **Courses** use cumulative rounds and points.
...
Opening identification supplies names and ECO codes rather than master-game statistics. Complete courses are organized from this database, with local rule-based move explanations and conditional positional plans; they are not individually authored master courses or engine-verified continuations. Teaching progress and the current engine game are stored separately from the study library. Engine play uses Stockfish’s actual limited-strength best move; review always remains full strength. Skill and Elo controls follow [Stockfish’s documented UCI options](https://official-stockfish.github.io/docs/stockfish-wiki/UCI-Protocol-and-Stockfish-Commands.html).
