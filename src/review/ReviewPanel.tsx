import { Star, Play, Square, ShieldCheck, GraduationCap } from 'lucide-react';
      ...
      <div className="review-highlights" aria-label="Analyzed move highlights">
        {(['Best', 'Excellent', 'Blunder'] as Label[]).map((label) => (
          <span key={label} style={{ color: labelInfo[label].color }}>
            <MoveQualityIcon label={label} size={20} />
            <strong>{done ? r.w.counts[label] + r.b.counts[label] : '—'}</strong> {label}
          </span>
        ))}
      </div>
      ...
      <div className="review-action">
        {reviewing ? (
          ...
          <button className="primary" onClick={onReview} disabled={!total}>
            ...
            {done === total && total ? 'Review again' : 'Game Review'}
          </button>
        )}
